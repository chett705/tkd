...
<div class="max-w-full mx-auto">
    ...
    <form action="{{ route('admin.section-items.update', $item->id) }}" method="POST" enctype="multipart/form-data" class="bg-white border border-slate-200 rounded-xl p-6 space-y-5 shadow-sm">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-2 gap-4 mb-4">
            <div>
                ...
                <select id="pageSelect" name="page" class="w-full border border-slate-200 mt-1 bg-slate-50 text-slate-700 p-2 rounded-xl">
                    ...
                    @foreach ($pages as $page)
                        <option value="{{ $page->slug }}" data-id="{{ $page->id }}" {{ $item->page == $page->slug ? 'selected' : '' }}>
                            {{ $page->name_en }}
                        </option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <input type="text" name="section_key" value="{{ old('section_key', $item->section_key) }}" class="w-full border border-slate-200 mt-1 bg-slate-50 text-slate-700 p-2 rounded-xl @error('section_key') border-red-500 @enderror" placeholder="services, projects, why-us">
            </div>
        </div>
        ...
        <div class="grid grid-cols-2 gap-4 mt-4">
            <input type="text" name="component_type" value="{{ old('component_type', $item->component_type) }}" placeholder="component_type..." class="border border-slate-200 bg-slate-50 text-slate-700 p-2 rounded-xl">
            <input type="text" name="type" value="{{ old('type', $item->type) }}" placeholder="Type" class="border border-slate-200 bg-slate-50 text-slate-700 p-2 rounded-xl">
        </div>

        <div class="grid grid-cols-2 gap-4 mt-4">
            <input type="text" name="title_en" value="{{ old('title_en', $item->title_en) }}" placeholder="Title EN" class="border border-slate-200 bg-slate-50 text-slate-700 px-2 py-2 rounded-xl">
            <textarea name="description_en" class="w-full border border-slate-200 bg-slate-50 text-slate-700 px-2 rounded-xl" placeholder="Description EN">{{ old('description_en', $item->description_en) }}</textarea>
            <textarea id="description_km" name="description_km" class="w-full border border-slate-200 bg-slate-50 text-slate-700 px-2 rounded-xl" placeholder="Description KM">{{ old('description_km', $item->description_km) }}</textarea>
        </div>
        ...
        <div class="grid grid-cols-2 gap-4 mt-4">
            <div>
                ...
                @if ($item->image)
                    <img src="{{ asset('storage/' . $item->image) }}" class="h-16 mt-2 rounded border border-slate-200">
                @endif
            </div>

            <div>
                <label class="text-xs text-slate-700">Icon</label>
                <input type="file" name="icon" class="w-full border border-slate-200 bg-slate-50 text-slate-700 p-2 rounded-xl">
                @if ($item->icon)
                    <img src="{{ asset('storage/' . $item->icon) }}" class="h-10 mt-2 rounded border border-slate-200">
                @endif
            </div>
        </div>
        ...
        <div>
            ...
            <div class="flex items-center justify-between mb-2">
                ...
                <span id="gallery-count-edit" class="text-xs text-slate-400">
                ...
                <div id="gallery-existing-edit" class="flex gap-2 overflow-x-scroll pb-2 mb-3" style="max-width:1136px">
                    @foreach (array_reverse($item->images, true) as $index => $img)
                        <div class="relative group flex-shrink-0 w-24">
                            <img src="{{ asset('storage/' . $img) }}" class="w-24 h-20 object-cover rounded-xl border border-slate-200">
                            <label class="absolute inset-0 flex items-end justify-center pb-1 opacity-0 group-hover:opacity-100 transition cursor-pointer">
                                <input type="checkbox" name="remove_images[]" value="{{ $index }}" class="hidden peer" onchange="toggleRemove(this)">
            ...
            <div id="gallery-drop-edit" class="border-2 border-dashed border-slate-300 rounded-xl p-5 text-center cursor-pointer hover:border-[#0d66b5] transition" onclick="document.getElementById('gallery-input-edit').click()">
                <svg class="mx-auto mb-2 w-6 h-6 text-slate-400" fill="none" stroke="currentColor" stroke-width="1.5" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M3 16.5v2.25A2.25 2.25 0 005.25 21h13.5A2.25 2.25 0 0021 18.75V16.5m-13.5-9L12 3m0 0l4.5 4.5M12 3v13.5" />
                </svg>
                <p class="text-slate-600 text-sm">Click or drag & drop to add more images</p>
                <p class="text-slate-400 text-xs mt-1">JPG, PNG, GIF, WEBP - total max 40</p>
                <input type="file" id="gallery-input-edit" name="images[]" multiple accept="image/jpeg,image/png,image/gif,image/webp" class="hidden">
            </div>

            <div id="gallery-preview-edit" class="mt-3 flex gap-2 overflow-x-scroll pb-2" style="max-width:1136px"></div>
            <p id="gallery-error-edit" class="text-red-500 text-xs mt-1 hidden">Maximum 40 images total allowed.</p>
        </div>

        <div class="grid grid-cols-2 gap-4 mt-4">
            <input type="text" name="link" value="{{ old('link', $item->link) }}" placeholder="Link" class="border border-slate-200 bg-slate-50 text-slate-700 p-2 rounded-xl">
            <input type="text" name="button_text_en" value="{{ old('button_text_en', $item->button_text_en) }}" placeholder="Button EN" class="border border-slate-200 bg-slate-50 text-slate-700 p-2 rounded-xl">
        </div>

        <div class="grid grid-cols-2 gap-4 mt-4">
            <input type="number" name="sort_order" value="{{ old('sort_order', $item->sort_order) }}" placeholder="Sort Order" class="border border-slate-200 bg-slate-50 text-slate-700 p-2 rounded-xl">
        ...
        <div class="mt-4">
            ...
            <textarea name="meta" class="w-full border border-slate-200 bg-slate-50 text-slate-700 p-2 rounded-xl" placeholder='{"key":"value"}'>{{ $item->meta ? json_encode($item->meta) : '' }}</textarea>
        </div>
        ...
        <div class="flex justify-end gap-2">
            ...
            <button class="px-4 py-2 bg-[#0d66b5] text-white rounded-xl hover:bg-[#0a4f97]">
                Update Item
            </button>
        </div>
...
@push('scripts')
    ...
    <script>
        ...
        function renderGalleryEdit() {
            ...
            [...selectedFilesE].reverse().forEach((file, ri) => {
                ...
                reader.onload = e => {
                    ...
                    div.innerHTML = `
                        <img src="${e.target.result}" class="w-24 h-20 object-cover rounded-xl border border-[#0d66b5]/40">
                        <button type="button" onclick="removeEditFile(${i})" class="absolute top-1 right-1 w-5 h-5 bg-red-500/80 hover:bg-red-500 rounded flex items-center justify-center opacity-0 group-hover:opacity-100 transition">
                            <svg class="w-3 h-3 text-white" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12"/>
                            </svg>
                        </button>`;
        ...
        galleryDropE.addEventListener('dragover', e => {
            e.preventDefault();
            galleryDropE.classList.add('border-[#0d66b5]');
        });
        galleryDropE.addEventListener('dragleave', () => galleryDropE.classList.remove('border-[#0d66b5]'));
        galleryDropE.addEventListener('drop', e => {
            e.preventDefault();
            galleryDropE.classList.remove('border-[#0d66b5]');
